...
$plumber = $_SESSION['admin'];
    ...
    while($row1 = mysqli_fetch_assoc($query1)){
        $sql2 = "SELECT * FROM messages WHERE (incoming_msg_id = {$row1['client_ID']}
                OR outgoing_msg_id = {$row1['client_ID']} ) 
        ...
        $output .= '<a href="chat.php?client='.$row1['client_ID'] .'">
                    ...
                    <div class="details">
                        <span >'. $row1['client_fname'].' '. $row1['client_mname'].' '. $row1['client_lname']. '</span>
                        <p>'. $you . $msg .'</p>
                    </div>
...
while($row = mysqli_fetch_assoc($query)){
    ...
    $output .= '<a href="chat1.php?client='.$row['plumber_ID'] .'">
