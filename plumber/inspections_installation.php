...
<section class="is-title-bar">
  <div class="flex flex-col md:flex-row items-center justify-between space-y-6 md:space-y-0">
    ...
    <li style='color:lightblue'>Plumber</li>
      ...
      <li style='color:darkblue'>Scheduled Installation</li>
    </ul>
			...
			<?php
		  		$id = $user['plumber_ID'];
						...
						LEFT JOIN tbladdress ON tbladdress.address_ID =  tblclients.address_ID
						 WHERE  inspection_schedule_for_registration_approval.inspected =0 AND inspection_schedule_for_registration_approval.inspection_installation =1 
						 AND inspection_schedule_for_registration_approval.sentToPlumber = 1 AND plumber_ID = '$id'  ";
				...
				?>
						<tr>
							...
							<td data-label="#"><?php echo $row['comment']?></td>
							<td data-label="#"><?php echo $row['tools']?></td>

							<td class="actions-cell">
						
							<form method='POST' action='inspections_installation.php?cmp=<?php echo $row['schedule_ID'];?>'>
...
if($cmp==true)
{
	...
	$sql = "UPDATE inspection_schedule_for_registration_approval SET inspected='1' WHERE schedule_ID='$id'";
	...
	$connect->query($sql);
		...
		"
		<script>
		window.location='inspections_installation.php';
		</script>
		";
	}
